                    ...
                    <div class="card-body">
                        <DataTable :custom-ref="table.customRef" :fields="table.fields" :slots="table.slots" :total-items="table.totalRows" :items="fetchData" :sort-by="table.sortBy">
                            <template slot="view" slot-scope="data">
                                <b-button variant="outline-info" :to="{name: 'event-dashboard', params: { id: data.tbl.item.event_id }}">
                                    <b-icon-eye/>
                                </b-button>
                            </template>
                        </DataTable>
...
export default {
    ...
    data() {
        return {
            title: "Mutually Agreeable Date List",
            items: [
                {
                    text: "Dashboard",
                    href: "/"
                ...
                {
                    text: "Mutually Agreeable Date List",
                    active: true
                }
            ],
            table: {
    ...
    methods: {
        fetchData(ctx, callback) {
            ...
            let url = 'events_list/create?filter=' + (ctx.filter ?? '') + '&sort-by=' + ctx.sortBy;
